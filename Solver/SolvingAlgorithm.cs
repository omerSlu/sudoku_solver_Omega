...
namespace Sudoku
{
    public class SolvingAlgorithm
    {
        ...
        */
         ...
         */
        public static int[,] Solve(int[,] board, int[,,] optionsMat)// this function needs to be split(brute force)
        {
            while (FillHidden(optionsMat, board) || FillNaked(optionsMat, board)) ;// try to solve by logic

            switch (SolveState(optionsMat))
            {
                case 1: // sudoku solved end timer and return board
                    return board;
                case -1:// path unsolvable try another option or declare board unsolvable
                    return null;
            }
            ...
            return SolveState(optionsMat) == 1 ? board : null;
        }
